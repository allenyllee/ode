...
#define WITH_DYNAMIC_ADJUSTMENT_STATS   1
...
void dxQuickStepIsland_Stage3(dxQuickStepperStage3CallContext *stage3CallContext)
{
    ...
    if (m > 0) {
        ...
        if (singleThreadedExecution) {
            ...
            dxWorld *world = callContext->m_world;
            dReal prematureExitDelta = world->qs.marginal_delta_values[MDK_PREMATURE_EXIT_DELTA];
            ...
            for (unsigned int iteration = 0, extra_num_iterations = 0; ; ) {
                ...
                if (iteration - extra_num_iterations == num_iterations) {
                    if (extra_num_iterations != 0 || world->qs.max_num_extra_iterations == 0) {
                        break;
                    }

                    extra_num_iterations = world->qs.max_num_extra_iterations;
                    prematureExitDelta = world->qs.marginal_delta_values[MDK_EXTRA_ITERATIONS_REQUIREMENT_DELTA];
                }
            ...
            dxWorld *world = callContext->m_world;
            stage4CallContext->m_LCP_iteration_premature_exit_delta = world->qs.marginal_delta_values[MDK_PREMATURE_EXIT_DELTA];
...
int dxQuickStepIsland_Stage4LCP_IterationStart_Callback(void *_stage4CallContext, dcallindex_t callInstanceIndex, dCallReleaseeID callThisReleasee)
{
    ...
    if (!abortIterating) {
        ...
        if (iteration + 1 - stage4CallContext->m_LCP_extra_num_iterations == num_iterations) {
            if (stage4CallContext->m_LCP_extra_num_iterations != 0 || world->qs.max_num_extra_iterations == 0) {
                lastIteration = true;
            }
            else {
                stage4CallContext->m_LCP_extra_num_iterations = world->qs.max_num_extra_iterations;
                stage4CallContext->m_LCP_iteration_premature_exit_delta = world->qs.marginal_delta_values[MDK_EXTRA_ITERATIONS_REQUIREMENT_DELTA];
            }
        }
